...
function getFirstIpAddress(cidrStr, callback) {
  ...
  let v6mappedAddress = null;
  let retAddr = new Object();
  ...
  if (!cidr.isValid()) {
     // If the passed CIDR is invalid, set an error message and return null data as retAddr.
     retAddr = {'ipv4':null,'ipv6':null}
    ...
    [firstIpAddress] = cidr.toArray(options);
     ...
     // Using the ipv4, map the ipv6 and build the object to return
    v6mappedAddress = getIpv4MappedIpv6Address(firstIpAddress);
    retAddr = {'ipv4':firstIpAddress,'ipv6':v6mappedAddress}
  }
  ...
  return callback(retAddr, callbackError);
}
